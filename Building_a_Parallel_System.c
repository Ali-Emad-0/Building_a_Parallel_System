...
void split_file(char *filename)
{
    ...
    for(int i=0;i<count;i++)
    {
        if(i%2==0)
        {
            #pragma omp critical(even_write)
            fputs(lines[i],even);
        }
        else
        {
            #pragma omp critical(odd_write)
            fputs(lines[i],odd);
        }
    }
...
void print_matrix(double M[N][N], const char *name)
{
    printf("\n===== %s (5x5 sample) =====\n", name);
...
int main(int argc,char** argv)
{
    ...
    {
        if(rank==0)
            printf("Run using:\nmpirun --oversubscribe -np 11 ./program\n");
        MPI_Finalize();
        return 0;
    }
    ...
    if(rank==0)
    {
        ...
        /* CUDA PROCESS */
        double GPU_A[N][N],GPU_C[N][N];

        for(int i=0;i<N;i++)
            for(int j=0;j<N;j++)
                GPU_A[i][j]=i+j;

        MPI_Send(GPU_A,N*N,MPI_DOUBLE,10,0,MPI_COMM_WORLD);
        MPI_Recv(GPU_C,N*N,MPI_DOUBLE,10,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
        ...
        print_matrix(C, "Matrix Addition Result");
        print_matrix(GPU_C, "CUDA Matrix Result");
    }
    ...
    {
        printf("CUDA worker started\n");

        double A[N][N],C[N][N];

        MPI_Recv(A,N*N,MPI_DOUBLE,0,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);

        cuda_matrix_double(&A[0][0],&C[0][0],N);
